  ...
  return (
    <Flex position={"relative"} w={"full"} h={"100vh"} flexDir={user?"row":"column"}>
        {pathname!=="/auth" && (user ? <Sidebar/>:<Header/>)}
            ...
            {user && <HeaderPage/>}
            <Flex justifyContent={"flex-start"} minH={"82%"} flexDirection={"column"} w={"full"}  px={3} py={3} roundedTop={"3xl"} roundedBottom={pathname=="/chat"?"3xl":""} bg={user?{base:useColorModeValue("gray.100","black"),md:useColorModeValue("white","gray.dark")}:useColorModeValue("white","black")} mt={user?{base:20,md:0}:20}>
                <Routes>
